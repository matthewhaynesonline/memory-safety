/**
 * MEMORY SAFETY VULNERABILITY DEMONSTRATION
 *
...
#define TEMPLATE_BUF_SIZE 8192

// ============================================================================
// Data Structures
// ============================================================================

// User structure with buffer overflow vulnerability
// Memory layout (on typical systems):
//   [username: 16 bytes][password: 16 bytes][is_admin: 4 bytes]
//
...
void free_user(User *user)
{
    ...
    printf("  Any sessions referencing this user now have DANGLING POINTERS!\n");
}
...
void expire_session(int session_id)
{
    for (int i = 0; i < MAX_SESSIONS; i++)
    {
        if (sessions[i] && sessions[i]->session_id == session_id)
        {
            ...
            printf("  Session freed, but user still exists at: %p\n", (void *)(current_session ? current_session->user : NULL));
...
{
    char *buf = malloc(TEMPLATE_BUF_SIZE);

    if (!buf)
    {
        send_response(client_socket, "500 Internal Server Error", "text/plain", "Template allocation failed");
        return;
    }
    ...
    int n = snprintf(buf + used, TEMPLATE_BUF_SIZE - used, "%s", template_header);
    if (n < 0 || (size_t)n >= TEMPLATE_BUF_SIZE - used)
    {
        free(buf);
        send_response(client_socket, "500 Internal Server Error", "text/plain", "Response too large for template buffer");
    }

    used += (size_t)n;

    n = snprintf(buf + used, TEMPLATE_BUF_SIZE - used, "%s", body_fragment);
    if (n < 0 || (size_t)n >= TEMPLATE_BUF_SIZE - used)
    {
        free(buf);
        send_response(client_socket, "500 Internal Server Error", "text/plain", "Response too large for template buffer");
    ...
    n = snprintf(buf + used, TEMPLATE_BUF_SIZE - used, "%s", template_footer);
    if (n < 0 || (size_t)n >= TEMPLATE_BUF_SIZE - used)
    {
        free(buf);
        send_response(client_socket, "500 Internal Server Error", "text/plain", "Response too large for template buffer");
    ...
    send_response(client_socket, status, "text/html", buf);
    free(buf);
...
{
    char *fragment = malloc(TEMPLATE_BUF_SIZE / 2);
    if (!fragment)
    {
    ...
    int n = snprintf(fragment, TEMPLATE_BUF_SIZE / 2, "<pre>%s</pre>", text);
    if (n < 0 || (size_t)n >= TEMPLATE_BUF_SIZE / 2)
    {
        free(fragment);
...
void handle_login(int client_socket, char *body)
{
    ...
    print_memory_layout("BEFORE processing login:", user);
    ...
    // Copy username safely (using the calculated length)
    ...
    // VULNERABILITY: No bounds checking on password
    // If password_len > 16, memcpy writes past the end of user.password
    // and into user.is_admin, allowing privilege escalation
    ...
    print_memory_layout("AFTER processing login:", user);

    // If there was a previous authenticated_user, free it (keeps demo behavior)
    ...
    char fragment[TEMPLATE_BUF_SIZE / 4];
    if (user->is_admin)
    {
    ...
    else
    {
        snprintf(fragment, sizeof(fragment),
                 ...
                 "  <p><a href=\"/check-user\">Check session/user</a></p>\n",
                 user->username, user->is_admin, (void *)user, session_id, (void *)current_session);
        send_html_template(client_socket, "200 OK", fragment);
        return;
    }
...
void handle_logout(int client_socket)
{
    ...
    snprintf(fragment, sizeof(fragment),
             ...
             "The User memory has been freed, but the Session still holds a pointer to it. This is a DANGLING POINTER!\n"
             "Any attempt to access current_session->user is undefined behavior!\n\n"
             "Visit /corrupt to allocate new data in the freed memory, then /check-user to see corruption!\n"
             "  </pre>\n",
             (void *)user_to_free,
             (void *)current_session,
             (void *)(current_session ? current_session->user : NULL));
...
{
    printf("\n===========================================\n");
    printf("CORRUPTING FREED MEMORY\n");
    printf("===========================================\n");
    ...
    // Allocate new memory - likely to reuse the freed User's address
    ...
    {
        printf("  Fastpath: allocator returned the same address immediately: %p\n",
               (void *)corrupted_data);
    }
    else
    {
        // Not the same — try allocator nudging
        free(corrupted_data);
        corrupted_data = NULL;
        ...
        for (int round = 0; round < MAX_ROUNDS && !matched; round++)
        {
            // Spray allocations to perturb heap/tcache state
            ...
            for (int i = 0; i < SPRAY_COUNT; i++)
            {
                ...
                if (!spray[i])
                    break;
                memset(spray[i], 0x41, s); // touch memory
            }

            // Free them all to refill tcache bins
            for (int i = 0; i < SPRAY_COUNT; i++)
            {
                if (spray[i])
                    free(spray[i]);
    ...
    snprintf(fragment, sizeof(fragment),
             ...
             "%s\n\n"
             "Now visit /check-user to see the corrupted data!\n",
             (void *)corrupted_data,
             target_addr,
             ((void *)corrupted_data == target_addr)
                 ? "✓ Memory reused at SAME ADDRESS - corruption likely visible!"
                 : "Note: Allocator used different address - corruption may not be visible.");

    send_html_template(client_socket, "200 OK", fragment);

    // Don't free corrupted_data - leave it allocated to keep corruption visible
}
...
void handle_check_user(int client_socket)
{
    ...
    if (!session_valid)
    {
        printf("  !  SESSION USE-AFTER-FREE DETECTED!\n");
    }
    if (!user_valid)
    {
        printf("  !  USER USE-AFTER-FREE DETECTED!\n");
        printf("  Attempting to read freed User memory...\n");
    }
    ...
    if (current_session && current_session->user)
    {
        // This is the dangerous part - reading from potentially freed memory
    ...
    snprintf(fragment, sizeof(fragment),
             ...
             "%s\n"
             "  </pre>\n",
             session_valid ? "" : "!  SESSION USE-AFTER-FREE! ",
             user_valid ? "✓ User is valid" : "!  USER USE-AFTER-FREE!",
             (void *)current_session,
...
void handle_secret(int client_socket)
{
    ...
    if (!u->is_admin)
    {
        send_text_in_template(client_socket, "403 Forbidden", "403 Forbidden: You are not an admin.\n<pre>TOP SECRET DATA\n  MATT ■■■ ■ ■■■■■■■■■ ■■■■■■■■■ ■■ ■■■ ■■■■! ■■■■■■■ ■■ ■■■■■■. ■■■■■■■■■■■</pre>");
        return;
    }
...
int main(void)
{
    ...
    printf("\n============================================\n");
    printf(" MEMORY SAFETY DEMO SERVER\n");
    printf(" http://localhost:%d\n", PORT);
    printf("============================================\n");
    printf("\nEndpoints:\n");
    printf("  GET  /            - Home page with examples\n");
    printf("  GET  /login       - Login form (HTML)\n");
    printf("  POST /login       - Vulnerable login endpoint (creates a Session automatically)\n");
    printf("  GET  /logout      - Logout and free the User (creates dangling pointer in Session)\n");
    printf("  GET  /corrupt     - Allocate new data to corrupt freed memory\n");
    printf("  GET  /check-user  - Access current session/user (may hit use-after-free)\n");
